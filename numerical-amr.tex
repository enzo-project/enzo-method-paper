...
\item{\em Optical depth.} This refinement criterion ensures that cells on the highest level of refinement have an HI optical depth lower than unity.  The optical depth of a cell is estimated to be:

\begin{equation}
\tau = \sigma_{HI} n_{HI} \Delta x 
\end{equation}

where $\sigma_{HI}$ is the neutral atomic hydrogen absorption cross section, $n_{HI}$ is the proper HI number density, and $\Delta x$ is the cell width.  If $\tau > 1$, the cell is refined further, up to the maximum level of refinement.
