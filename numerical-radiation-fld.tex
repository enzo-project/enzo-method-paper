...
\end{eqnarray}
where $E_r$ is a grey radiation energy density, $e_c$ is the internal
energy correction due to photo-heating and chemical cooling, and
${\mathrm n}_i$ correspond to the chemical number densities of
\ion{H}{1}, \ion{H}{2}, \ion{He}{1}, \ion{He}{2} and \ion{He}{3}, and
${\mathrm n}_e$ is the electron number density.  Here, we define the
grey radiation energy density through first assuming a fixed frequency
spectrum, i.e.  $E_{\nu}(\nu,\vec{x},t) = \tilde{E}_r(\vec{x},t)
...
(\ref{eq:fld_radiation}-\ref{eq:fld_chemistry}) is solved
...
\begin{eqnarray}
  ...
  \partial_t e_c &=& -\frac{2\dot{a}}{a} e_c +
    G\left(\bar{E}_r,\bar{\mathrm n}_i\right) - 
    \Lambda\left(\bar{E}_r,\bar{\mathrm n}_i\right), \\
  \label{eq:fld_chemistry_qss}
  \partial_t {\mathrm n}_i &=& \alpha_{i,j}\left(\bar{e}_c\right)
