...
%\begin{equation}
%\frac{\partial \rho_b}{\partial t} + \frac{1}{a} \vecv_b \cdot \grad \rho_b =  \label{enzoconserve}
% - \frac{1}{a} \rho_b \div \vecv_b 
%\end{equation}

%\begin{equation}
%\frac{\partial \vecv_b}{\partial t} + \frac{1}{a} (\vecv_b \cdot \grad) \vecv_b = \label{enzomomentum} 
%-\frac{\dot{a}}{a} \vecv_b - \frac{1}{a \rho_b} \grad p - \frac{1}{a} \grad \phi
%\end{equation}

%\begin{eqnarray}
%\frac{\partial E}{\partial t} + \frac{1}{a} \vecv_b \cdot \grad E =  
%- \frac{\dot{a}}{a} \left( 3 \frac{p}{\rho_b} + \vecv_b^2 \right)  \nonumber \\
%- \frac{1}{a \rho_b} \div (p \vecv_b) \nonumber \\
%- \frac{1}{a} \vecv_b \cdot \grad \phi + \Gamma - \Lambda
%\label{enzoenergy}
%\end{eqnarray}
...
%\begin{equation}
%\frac{d \vecv_{part}}{dt} = -\frac{\dot{a}}{a} \vecv_{part} - \frac{1}{a} \grad \phi
%\label{enzopartmom}
%\end{equation}
...
of space in its \emph{parent grid} that requires higher resolution,
...
%%% If you prefer the following \paragraph{} sections to be numbered,
%%% then you can add \setcounter{secnumdepth}{5} to the preamble...
...
\paragraph{MHD with Constrained Transport (MHD)}
...
hydrodynamical algorithm of \zeus\ has been implemented in \enzo\ --
the MHD and RHD schemes have not. The \zeus\ method uses a staggered
mesh such that the velocities are face-centered, while the density and
internal energy quantities are cell-centered.  It splits the solution
...
interpolate the boundary conditions from the parent grid (either the
level 0 grid or some other subgrid). The Poisson equation is then
solved on every timestep using a multigrid technique on one subgrid at
a time. Aside from calculating the gravitational potential arising
from the baryon fields and particles in the simulation, there are also
a number of options for specifying static gravitational fields, and in
...
populated with lower mass particles in order to also improve the
...
(except for the chemistry step, which is sub-cycled) separately, the
minimum $\Delta t$ allowed for stability. The level is advanced with a
timestep equal to the minimum over all of these $\Delta t$.
